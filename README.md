...
| `/promote`         | Run the full PropPlane keeper ladder promote - agent sandbox to `prakrit`, `main`, and `production` - behind the security review and no-mistakes gates, leaving a promotion-record PR for the `prakrit` -> `main` step; needs this home's PropPlane ladder config |
